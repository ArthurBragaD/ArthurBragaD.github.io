...
<body>
    ...
    if ($_GET["fazer"] === "excluir") {
        ...
        $sql = "DELETE FROM EditalArquivos WHERE idArquivo =" . $id;
        $db->exec($sql);
        $db->close();
        header('Location: MudaEditalArquivo.php?id=' . $idEdital);
    };
    if (isset($_POST["atualizar"])) {
        if (!empty($_POST["nome"]) && !empty($_POST["hora"]) && !empty($_POST["tipo"]) && $_FILES["arquivo"]["size"] != 0) {
            ...
            if (in_array($ext, $ext_allowed)) {
                if (move_uploaded_file($file_temp, $localizado)) {
                    ...
                    $sql = "UPDATE EditalArquivos SET nome ='".$nome. "', hora ='" . $hora . "', tipo ='" . $tipo . "', arquivo ='" . $arquivo . "', localizado ='" . $localizado. "' WHERE idArquivo ='".$id."'"; 
                    echo $sql;
                    $db->exec($sql);
        ...
        } elseif (!empty($_POST["nome"]) && !empty($_POST["hora"]) && !empty($_POST["tipo"])){
            $db = new SQLite3('../db/userData.db');
            $id =  $_POST["id"];
            $nome = $_POST["nome"];
            $hora = $_POST["hora"];
            $tipo = $_POST["tipo"];
            $sql = "UPDATE EditalArquivos SET nome = '" . $nome . "', hora = '" . $hora . "', tipo = '" . $tipo . "' WHERE idArquivo ='" . $id."'";
            echo $sql;
            $db->exec($sql);
            echo "Arquivo do edital catalogado";
            $nome = "";
    ...
    if ($_GET["fazer"] === "modificar") {
        $id = $_GET["id"];
        $db = new SQLite3('../db/userData.db');
        $sql = "SELECT * FROM EditalArquivos WHERE idArquivo =" . $id;
    ...
    ?>
    <div>
        <form action="./MudaEditalArquivo.php">
            <input type="hidden" name="id" value="<?php echo $idEdital; ?>">
            <button type="submit" class="btn btn-primary voltarbutton">Voltar</button>
        </form>
    </div>
    <div class="titulo-container">
        <h2 class="titulo-conteudo">Modificar Arquivos do Edital</h2>
    </div>
        ...
        <form action="" enctype="multipart/form-data" method="POST" class="form-container">
            ...
            <ul>
                <li style="width: 50%;margin-left: 25%;margin-right:25%">
                    ...
                    <span>Coloque o nome do arquivo do edital</span>
                </li>
                <li style="width: 50%;margin-left: 25%;margin-right:25%">
                    <label for="hora">Data</label>
                    <textarea onkeyup="ajusta_texto(this)" name="hora" value="<?php echo $dados["hora"]; ?>"><?php echo $dados["hora"]; ?></textarea>
                    <span>Coloque o data de liberação do arquivo</span>
                </li>
                <li style="width: 50%;margin-left: 25%;margin-right:25%">
